...
- `.github/workflows/release.yml` `create-release` job — bash `gh release create` → `softprops/action-gh-release@v2` with `generate_release_notes: true`. Auto-generated "What's Changed" body matches standard Rust CLI repo conventions (ripgrep / starship / etc.).
