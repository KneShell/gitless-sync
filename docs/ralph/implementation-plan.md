...
- [~] **A**: spec-output-schema.md § v1.4 → v1.5 변경 § 본문에 Finding 1 면제 근거 한 줄 추가 — "v1.4 caller `files == null` / key 부재 분기는 v1.4 시점 도입된 신규 가정이라 SemVer 보호 대상 아님". `minor bump` 라벨 유지 + 면제 근거 명문화 둘 다. Files: docs/specs/spec-output-schema.md.
