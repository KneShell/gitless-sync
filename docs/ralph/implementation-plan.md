...
## Status
- Last updated: 2026-04-30T00:00:00Z (T13 obsoleted by ADR 0001; v0.1 plan effectively complete)
- Total tasks: 16 (T13 obsolete)
- Completed: 15 / 15 active
...
L4 (validate): T11 ── T12 (BLOCKED, T14 unblocks)
                |       ↑
                |      T14 (cleanup, no deps)
L5 (human):    T13 [OBSOLETE — ADR 0001]
```
...
### T13. [OBSOLETE — ADR 0001] 실제 GitHub repo + Fine-grained PAT 통합 검증
- **Status**: `[-]` OBSOLETE (2026-04-30, ADR 0001).
- **사유**: `docs/adr/0001-gh-subprocess-and-drop-push-tool.md`로 gh CLI subprocess 채택 결정. 인증 책임이 도구 외부(gh)로 이동했으므로 도구가 PAT 최소 권한을 검증·문서화할 필요 없음. 사용자는 `gh auth login` 한 줄로 인증 완료.
- **남는 가치**: vault 실전 검증 (2026-04-29, OAuth token via `gh auth token`, 356 파일, 0 drift)로 도구 동작은 이미 입증됨. 추가 검증 불필요.
- **Historical refs**: `docs/specs/spec-github-api.md` § Open Question, `docs/roadmap.md` § Open Questions (둘 다 OBSOLETE 표기).
