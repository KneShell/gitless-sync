...
- [x] **Y. encoding_rs binary size 사후 측정**
  ...
  - spec: 없음 (research artifact).
  - 결과 (2026-05-09): `cargo bloat 0.12.1` + `objdump -h` 측정 — encoding_rs `.text` section attribution **0 bytes** (top 50 crates 외 + `--filter encoding_rs` 0 KiB, LTO + strip + dead code elim). 전체 `.exe` 2,476,032 bytes (`.text` 1.9 MiB / `.rdata` 393 KiB / 합 2.4 MiB). README "fairly large" + ~1MB 가정은 unconditional 박음 가정 — LTO 박힌 4 encoding shortlist (Shift_JIS / EUC_KR / GBK / Windows-1252)만 retain 박혀 정량 부정. **Option A 결정 영향 0** (~1MB 정량 부정 + delta 측정 한계 미만, encoding_rs 단독 채택 confirmed). `cargo tree -i encoding_rs` 박음 — gitless-sync 직접 dep + transitive 0건. advisor 권고 박음 dual-build skip (Phase 5 다른 dep 노이즈 + 빌드 시간) — `.rdata` 정적 테이블 attribution은 dual-build 박음 박야 정확하나 전체 `.rdata` 자체가 393 KiB라 ~1MB 가정 정량 부정에는 충분. `docs/research/encoding-library-eval.md` § Y task 결과 (2026-05-09) section 신설 + § 라이브러리 후보 line 33 `[unverified]` → measured value + § Limitations item 3 `[unverified]` → verified + § 후속 task 박음 입력 § Y task → [x] marker 갱신. Phase 6+ `working-tree-encoding` 지원 시 또는 vault dogfooding (T task)에서 shortlist 확장 cascade 시 재측정 트리거 박음. validation: cargo fmt clean + clippy 0 warnings + xtask check-line-limits (52 + 5 within 300) + xtask check-cycles (0/0) + cargo machete clean + cargo test 49 xtask pass (lib + integration baseline 유지). 코드 변경 0 — G-012 spec-only 면제 적용 (tarpaulin baseline 유지 90.73%). 다른 task scope 침범 없음.
...
- [x] **V. CLAUDE.md / roadmap.md 완료 박스 박음**
