...
`docs/specs/spec-architecture.md` § 외부 도구 정의. cycle/LOC/API/unused dependency 게이트 적용 시 사용.
...
- `cargo machete` — unused dependency 검출. Exit 0 = clean, exit 1 = 위반 발견. **현 baseline**: 위반 0건. 정 false positive 시 `[package.metadata.cargo-machete] ignored = [...]`로 명시.

### 게이트 적용 시점

| 도구 | xtask wrap | enforcement |
|---|---|---|
| cargo-modules | `cargo xtask check-cycles` | deny active (cycles + cross-slice refs, baseline 0 위반) |
| (없음 — pure xtask) | `cargo xtask check-line-limits` | deny active (300 LOC, baseline 0 위반, doc-heavy 면제) |
| cargo-public-api | (직접 실행, CI 비교) | PR 이벤트에서 `diff origin/<base>..HEAD` 표시 (deny 아님) |
| cargo-machete | (직접 실행, CI gate) | CI gate active (exit 1 deny, baseline 0 위반) |

상세 권장 cli/exit 정책은 spec/plan 변경 시 동반 갱신.
