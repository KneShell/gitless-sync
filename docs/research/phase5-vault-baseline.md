...
- schema_version: 1.0 (`mode` / `failed_reason` / `lfs_pointer` 필드는 task O 시점 1.1 박힘)
...
이 2건은 **도메인 함정이 아님** — scan 명령 자체의 부산물이라 vault 분석 의미는 0. 실 vault에서는 scan 출력을 `tmp/`가 아닌 다른 디렉토리에 redirect하거나 pipe로 처리하면 자동 제거.
...
- Phase 5 처리 후 vault dogfooding은 task T가 담당. 본 task A는 baseline snapshot + fact check만 박음.

## Fact Check Sub-Steps

### 1. encoding_rs binary size

**Status**: deferred to task Y (cargo-bloat dry-run + dependency tree, encoding_rs 채택 후 측정). 본 task A는 baseline 박음.

**Current baseline** (Phase 5 진입 직전, encoding_rs 미박힘):
...
- encoding_rs는 미박힘 (`cargo tree -p encoding_rs` → 패키지 없음 확인)
...
- 정적 인코딩 테이블이 커서 stripped binary delta가 보통 ~1MB 수준으로 보고됨 (참조: encoding_rs README, "encoding_rs is fairly large" 명시) [unverified — Y task에서 cargo-bloat 정량 측정 박음].
...
`cargo-bloat` 미설치 상태로 본 task A baseline 측정만 박고, encoding 라이브러리 채택(E task) → 정량 측정(Y task) cascade에서 정확 delta 박음.
...
`.gitattributes`에 `binary` / `text=auto` 명시된 file은 본 휴리스틱 무시 — K1.5/K2 task 박힘 정합. 미명시(default) file은 v0.1 그대로 NUL 휴리스틱 적용 (spec-domain-pitfalls.md § 화이트리스트 표 default row).

### 3. Windows NTFS NFC/NFD 파일 공존 검증

**Hypothesis** (spec-domain-pitfalls.md § 검증 환경): NTFS는 normalize 안 함 — UTF-16LE 그대로 저장 → NFC/NFD 다른 byte sequence는 directory에서 별도 entry로 박힘. compose 한글(`\u{AC00}`) vs decompose(`\u{1100}\u{1161}`) 둘 다 같은 directory 공존 가능.
...
- File count: **2** — 둘 다 별도 entry로 공존 박힘
...
→ **Hypothesis 정합**. NTFS는 두 form을 별개 path로 박는다 — NFC/NFD 정합 처리 안 하면 vault 운영 환경(macOS APFS NFD ↔ Windows NTFS NFC `core.precomposeunicode` 차이)에서 false drift 직접 surface 가능.

**함의 (Phase 5 task C/D1)**:
- task C (NFC 정규화)는 walker에서 둘 다 catch한 뒤 NFC로 통일하면 같은 key — 하지만 둘 다 실파일이 박힌 directory에서는 NFC 충돌 발생 → spec-domain-pitfalls.md § Path 정규화 "NFC 정규화 후 같은 key 충돌 → `Status::Failed` + `failed_reason: "nfd_collision"`" 정책 적용.
- task P1 (NTFS 실파일 fixture)은 본 실험을 그대로 `tempfile` integration test로 박으면 됨 — PowerShell 의존 없이 Rust `std::fs::File::create` + 명시 path bytes로 재현 가능.

## Pitfall Priority Input

vault 데이터 부재로 본 task의 우선순위 박음 입력은 제한적. task B에서 박을 우선순위는 **이론 + spec 매핑 + fact check** 3축으로 전개:

| 함정 | 본 task baseline | task B 박음 시 우선순위 근거 |
|---|---|---|
...
| 비-UTF-8 인코딩 | encoding_rs 미박힘 (size delta unknown) | EUC-KR vault에서 2순위 — 변환 시도 + raw bytes hash (b) 정책 |
| submodule | 미surface | 일반 vault에서는 0건 가능성 높음, detect-only로 충분 |
| symlink | 미surface | Windows unprivileged 박을 수 없으므로 mock 검증 |
...
| `.gitattributes` | 미surface (KneShell/gitless-sync 없음) | 화이트리스트 박힌 vault에서 1순위 정확화 (큰 변경) |
...
→ **task B는 vault 데이터 없는 상태에서 박음**. 실 vault 접근 시점에 추가 검증 task 박음 가능 (W task 회귀 diff에서 catch).

## Limitations

1. **Vault 접근 불가**: 머신 환경 한정 (C:\Users\dasgut, vault는 `C:\Users\admin` 경로). Phase 5 진행 중 vault 접근 환경 박히면 본 task 결과를 task T에서 추가 baseline으로 갱신 가능.
2. **encoding_rs binary delta 미측정**: cargo-bloat 미설치 상태. Y task에서 cargo-bloat 박고 정량 측정.
3. **schema_version 1.0 한계**: `mode` / `failed_reason` / `lfs_pointer` 필드 미박힘 — 본 baseline은 v0.1 4분류 한정. task O 시점 1.1 박힘 → 본 baseline 비교 시 schema diff 정확화로 분류 (W task 자동 비교).

## Acceptance

- [x] vault scan 재실행 (KneShell/gitless-sync 한정, vault 부재) — 92 files baseline 박음
- [x] drift 근원 분석 — 2건 모두 scan 자체 race noise, 함정 0건 surface
- [x] encoding_rs binary size baseline — 2.30 MB (encoding_rs 미박힘 baseline), Y task에서 delta 측정
...
- [x] Windows NTFS NFD 파일 생성 검증 — NFC/NFD 공존 박음 직접 확인 (위 §3)
