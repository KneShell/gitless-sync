...
> 작성: 2026-05-09 (task K). 본 프로젝트의 300 LOC 임계 박힘(`docs/specs/spec-architecture.md` § LOC 임계, 사용자 취향 박제 2026-05-08)를 외부 통계와 비교하는 **post-hoc sanity check** artifact. 임계를 도출한 자료가 아니라 합리성 사후 점검용 — 결정은 인지부하 기반 구조적 선택, 본 통계는 그 선택이 성숙한 Rust 생태계 기준 어디 위치하는지만 측정.
...
- 결론: 우리 300 임계는 성숙한 Rust 프로젝트가 자연 도달하는 분포 대비 **엄격한 편**. 그러나 "비현실적으로 엄격"하지는 않음 — 모든 비교 프로젝트가 ≤ 200 LOC bin에 가장 많은 file을 두고 있어, 임계 자체는 자연 분포 안에 있음. 우리가 0%로 박은 건 enforcement 결과지 분포 정상값이 아님.
- **임계 박힘은 외부 stats가 아닌 인지부하 + 구조적 선택**. 본 문서는 "기괴하지 않다"의 사후 확인일 뿐.
...
즉 본 임계 300은 "엄격한 편이지만 자연 분포 안에 있는 위치". ≤ 300 LOC 비중은 ripgrep 57% (29+22+6) ~ cargo 81% (68.7+7.9+4.3) 범위, 4개 프로젝트 median 약 75%. 즉 우리 임계는 외부 자연 분포의 다수 sub-set과 정렬됨 — 비-자연 위치가 아님. 우리는 enforcement로 100%를 임계 아래로 박은 것.
...
비교 프로젝트들은 LOC 임계를 enforce하지 않음. file 크기가 자연스럽게 결정됨 — 도메인 복잡도, integration test 통합도, 모듈 분리 비용에 따라. 본 프로젝트는 의식적 제약을 박았기에 분포가 인위적 — 임계 직전(270–288)에 다섯 file이 몰려있음 (top-5).
