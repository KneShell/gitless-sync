...
`cargo xtask check-cycles` (xtask가 `cargo modules dependencies --lib --no-fns --no-types --no-traits --no-sysroot` DOT 출력을 파싱해 module-level uses 그래프에서 cycle 검출). cargo-modules `--acyclic`은 type-method edge를 cycle로 잘못 잡는 false positive가 있어 직접 사용하지 않음.

### Slice-internal directional discipline

각 slice 안 file은 다음 방향성을 따른다:
...
강제 메커니즘: naming convention + `pub(crate)`/`pub(super)` 가시성. 별도 manifest 도입 안 함 (yagni — 작은 CLI에 deviation 거의 없음).
...
- **`mod.rs`**: re-export hub 또는 thin orchestrator. LOC 게이트 자연 통과.
- **sub-module 간 sibling cross-ref 허용**: `use super::parser::Rule;` 같은 직접 참조. Rust 관용 정합.
- **acyclic 강제**: `cargo xtask check-cycles` 게이트가 sub-module 단위에도 자연 적용.
- **directional discipline 적용 X**: 단일 책임이라 sub-module 간 같은 layer 자연.

근거: `std`/`tokio`/`serde` 등 큰 crate도 sub-module 간 sibling cross-ref 자연 사용. mediator 강제하면 mod.rs 비대화 + LOC 300 게이트 위반 위험.

### Horizontal layer 영구 제외

CLI/Domain/IO 전체 분층은 채택 안 함. vertical slice 정책과 충돌 + 작은 CLI에 oversize.
...
- **mod.rs re-export only**: 자연 통과 (re-export만 포함된 mod.rs는 30~50줄 안).
...
직교성 — 셋 다 인지부하 frame이지만 서로 다른 escape hatch 차단:
- cog_comp = 함수 단위 branching 깊이 (짧지만 dense)
- too_many_lines = 함수 단위 LOC (길지만 flat)
- LOC 300 = file 단위 sprawl (잘 분리된 짧은 함수가 모여 file 비대화)

한 쪽 제거 시 다른 쪽이 못 잡는 패턴이 통과 — 결정 근거는 ADR 0010.

### 면제 카테고리

- **test 코드**: `#[cfg_attr(test, allow(clippy::unwrap_used, clippy::expect_used, clippy::panic))]` 면제는 panic 검출 lint 한정. cog_comp / too_many_lines는 test 코드도 그대로 적용 — 모놀리식 test 회피.
...
| 도구 | 목적 |
|---|---|
| `cargo-modules` | 의존 그래프 + cycle 검출 |
| `cargo-public-api` | API 변경 추적 (분할 회귀 가드) |
| `cargo-machete` | unused dependency |
| `cargo-tarpaulin` | coverage ≥ 80% (LLVM 백엔드) |
...
채택 안 함. 도메인 사실 — CLI 1회 호출 → main.rs dispatch → 단일 명령어 실행 → 종료. cross-feature 런타임 통신 0. 참조 방향성 보호는 위 Layer 정의 (cross-slice 금지 + acyclic + directional discipline + 가시성)로 강제.
