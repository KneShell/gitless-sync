...
- `text=auto eol=...` 복합 — eol만 적용, text=auto 전체는 화이트리스트 외
...
- 추가: integration test fixture로 실제 0-byte 파일 사용 + local ↔ remote identical 분류 검증.
...
- **detection 경로 — `.gitattributes` 파싱 시점**:
  ...
  - scan은 blob fetch 안 함 (Phase 4 GraphQL batching 이득 유지). LFS-tracked path는 자동 `Status::Failed` + `failed_reason: "lfs_pointer"` + `lfs_pointer: {oid: "?", size: 0}` (oid/size는 blob fetch 안 했으므로 unknown).
- **Defence-in-depth (옵션)**: `diff` 명령은 이미 blob fetch — pointer text 첫 줄 `version https://git-lfs.github.com/spec/v1` 시그니처 추가 검증. oid + size 정확히 파싱.
...
- diff 명령은 blob fetch를 사용하므로 추가 시그니처 검증으로 pointer text 정확 파싱 가능.

### Windows long path / 예약 파일명

- 260자+ path: `\\?\` prefix 없는 path는 Windows API 한도. local walker fail.
...
- 정책: drift로 판정 안 함, content 같으면 `Status::Identical`. JSON `mode` 필드에 포함 (호출자가 mode 차이 정책 결정).
...
**정확화 (의도된 변화 — 회귀 아님)**:
- v0.1에서 우연히 `Identical`로 분류된 binary가 v0.2에서 mismatch (`drift` 또는 `failed`)로 변경 — `.gitattributes` 정확 재현으로 정확화.
- v0.1에서 LFS pointer를 raw text로 처리해 mismatch한 entry가 v0.2에서 `failed_reason: "lfs_pointer"` 명시.
- v0.1에서 NFC/NFD 다른 path key로 분류돼 false drift였던 entry가 v0.2 NFC 정규화로 `Identical`.

**회귀 (예상 외 변화 — 차단)**:
- 위 정확화 화이트리스트 외 status 변화.
- 새 `failed` 발생인데 reason이 spec § failed_reason enum 외.
- schema_version 변경됐는데 v1.0 호출자 파싱 실패 (backward-compat 위반).
...
- v0.1 결과 ≠ v0.2 결과 무조건 발생 (text=auto / binary / eol 분기 활성).
- 정확화로 분류, 회귀 아님.
...
- submodule도 mock 응답으로 검증 (실제 submodule 사용 안 함).
- 인코딩 fixture는 byte literal로 EUC-KR / Shift_JIS / Latin-1 사용.
- LFS pointer fixture는 byte literal로 작성.
...
### 우선순위 결정 (이론 + spec 매핑 + fact check 3축)

vault 데이터 부재 + KneShell/gitless-sync surface 0건이라, 우선순위 입력은 (1) spec § 함정 처리 정책 매핑 + (2) task A fact check 3건 + (3) downstream task 결과 누적의 3축으로 결정.
...
| **P1** | `.gitattributes` | KneShell/gitless-sync 부재 | 화이트리스트 적용된 vault에서 1순위 정확화 (큰 변경 — conditional normalize) |
| **P1** | LFS pointer | 미surface | 미디어 vault에서 1순위 — `.gitattributes filter=lfs` 매칭 path 자동 fail mark |
| **P2** | 비-UTF-8 인코딩 | encoding_rs 미구현 (size delta unknown — Y task) | EUC-KR vault에서 2순위 — 변환 시도 + raw bytes (b) hash |
...
| **P3** | symlink (`120000`) | 미surface (Windows unprivileged 생성 불가) | mock 검증으로 cover |
...
본 표의 P0~P3 등급은 **함정별 우선순위 정보**(vault 운영 시 false drift 영향 크기 + 정확화 필요성)이고, 실제 task implement 순서는 `docs/ralph/implementation-plan.md` § 의존 순서가 정의 (acceptance 의존 + spec cascade + fixture 추가 시점). 등급과 implement 순서는 1대1 매핑 아님.
...
- 함정별 unit test fixture 추가 (Windows 환경 — raw bytes injection / NTFS 실파일 / mock 응답).
...
- tarpaulin 80% 게이트 유지 — Phase 5 새 코드 cover 정책 적용.
- CHANGELOG.md v0.2 작성 (V1 task).
