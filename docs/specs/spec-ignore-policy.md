...
- 의존성: `ignore` crate (`Cargo.toml`에 추가됨) + `walkdir`.
...
ignore 매치된 path는 **scan 결과 어디에도 포함하지 않는다**. 정확한 의미:

1. **walker 산출물 제외**: `walk(root, matcher)` 가 반환하는 `Vec<LocalFile>` 에 포함 안 함 (`walker.rs`).
2. **비교 대상 제외**: `compare`/`classify` 입력에 포함 안 함 — 4분류 판정 자체가 일어나지 않음.
3. **summary 카운트 제외**: `Summary { identical, local_only_changed, remote_only_changed, drift, failed }` 어느 카운터에도 +1 하지 않음 (`output.rs::Summary`). ignored 카운터는 별도 추가하지 않는다 — scan 범위 자체에서 사라짐.
4. **JSON `files[]` 제외**: `--include-files` 지정해도 ignored path는 entry로 직렬화되지 않음.
...
**원격 측 정책**: GitHub Trees API 응답의 path는 ignore 적용 안 함 (원격은 git 관리 하 — `.git/` 같은 path가 원격에 존재할 일 없음). 단 `--ignore` CLI 패턴은 원격 path도 동일 매처로 거름 (양쪽 합집합 동률 보장 — local-only 또는 remote-only 분류가 ignore 패턴으로 인해 비대칭 발생을 방지).
...
- `[AUTO]` builtin 패턴 매치: `is_ignored(Path::new("target/debug/build/foo"))` == `true` (Phase 5 — `target/` 검증 추가, `ignore::tests::builtin_matches_target_subpath`).
...
- `[AUTO]` Phase 5 — custom `.gitignore` 패턴 walk 적용: `.gitignore`에 `build/` 추가 → `build/` 하위 file이 walker 산출물에 포함 안 됨 (`walker::tests::applies_gitignore_from_root`).
